...
import { WorksiteResponseDto } from '../model/WorksiteResopnseDto';
...
export class WorksitesService {
  ...
  getWorksites(
    ...
    if (startDate && !roomId && !endDate) {
      ...
      return this.http
        .get<WorksiteResponseDto[]>(
          this.GET_WORKSITES + startDateSearch + start
        )
        .toPromise();
    }
    if (startDate && endDate && !roomId) {
      ...
      return this.http
        .get<WorksiteResponseDto[]>(
          this.GET_WORKSITES + endDateSearch + end + startDateSearch + start
        )
        .toPromise();
    }
    if (roomId && !startDate) {
      floorSearch = '?roomId=';
      return this.http
        .get<WorksiteResponseDto[]>(this.GET_WORKSITES + floorSearch + roomId)
        .toPromise();
    }
    if (roomId && startDate && !endDate) {
      ...
      return this.http
        .get<WorksiteResponseDto[]>(
          this.GET_WORKSITES + floorSearch + roomId + startDateSearch + start
        )
        .toPromise();
    }
    if (roomId && startDate && endDate) {
    ...
    return this.http.get<WorksiteResponseDto[]>(this.GET_WORKSITES).toPromise();
  }
